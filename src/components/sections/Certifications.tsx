...
const Certifications = ({ searchQuery }: CertificationsProps) => {
  const filteredCertifications = useMemo(() => {
    ...
    return certificationsData.filter(cert => {
      ...
      const nameMatch = cert.name.toLowerCase().includes(searchLower);
      ...
      <div className="container max-w-4xl">
        ...
        <div className="space-y-4">
          {filteredCertifications.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground">No certifications found matching your search.</p>
          )}
        </div>
      </div>
    </section>
